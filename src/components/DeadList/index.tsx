...
const DeadList: FC<{ dead: { player: string, attacker: string }[], players: { publicKey: string, name: string }[] }> = ({ dead, players }) => {

  const deadPublicKeys = useMemo(() => dead.map(d => d.player), [dead]);
  return (
    <Flex justifyContent="center" position="relative" maxW={'21.25rem'} w={'100%'} pt={'3rem'} pb={'1.875rem'} bg="button.border" px="1.25rem" borderRadius="1rem" border="0.375rem" borderStyle="solid" borderColor="button.bg">
      ...
      <Flex w={'100%'} flexDirection={'column'} gap={'1rem'}>
        ...
        >
          {deadPublicKeys.map((publicKey, idx) => (
            <Flex
              key={publicKey}
              w={'100%'}
              fontSize={'1.5rem'}
            ...
            >
              ...
              <Box borderRightRadius={'0.5rem'} bg={'#fff'} flex={1} lineHeight={1} p={'0.5rem 0.75rem'}>
                {players.find(player => player.publicKey === publicKey)?.name ?? publicKey}
              </Box>
            </Flex>
          ))}
        </VStack>
